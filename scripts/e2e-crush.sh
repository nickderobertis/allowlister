...
note "» case 1/5: shell deny — \`touch\` must be blocked even though \`crush run\` auto-approves the session"
...
note "✓ crush live e2e passed (shell deny+allow, dynamic plugin deny, built-in read deny, MCP deny)"
