...
#
# Equivalent environment variables: ALLOWLISTER_VERSION, ALLOWLISTER_INSTALL_DIR.
...
# use `cargo install allowlister --locked`.
#
# Like the engine it installs, this script never weakens silently: it aborts
# rather than install a binary it cannot checksum-verify.

set -eu

REPO="nickderobertis/allowlister"
BIN="allowlister"
# Overridden to `allowlister.exe` on Windows targets by detect_target.
BIN_FILE="$BIN"
...
usage() {
    cat >&2 <<EOF
Install the prebuilt allowlister binary.
...
Environment: ALLOWLISTER_VERSION, ALLOWLISTER_INSTALL_DIR, GITHUB_TOKEN.
EOF
}
...
main() {
    version="${ALLOWLISTER_VERSION:-}"
    bindir="${ALLOWLISTER_INSTALL_DIR:-}"
    ...
    # extension with `.sha256` (not appending), e.g. allowlister-v0.1.0-<t>.sha256.
    sumfile="${BIN}-${version}-${TARGET}.sha256"
    base_url="https://github.com/$REPO/releases/download/${version}"

    tmp="$(mktemp -d 2>/dev/null || mktemp -d -t allowlister)" \
