...
the Qwen Code CLI; Copilot next) and decides whether to **allow**, **deny**, or
...
For **Qwen Code** (`--harness qwen`) it merges the `PreToolUse` hook into
`~/.qwen/settings.json` (or `./.qwen/settings.json` for a project), preserving your
other Qwen settings. The hook fires in every approval mode, including `--yolo`, so
an allowlister **deny** is authoritative even in an unattended run. Qwen has no
allow list to broaden, so there is no allow-list warning.
...
allowlister init --local  --harness qwen        # project config, Qwen Code hook registered
...
allowlister hook <harness>        Read hook JSON on stdin, write a decision on stdout.
                                  Harnesses: claude-code, cursor, qwen (copilot is a stub).
...
allowlister init [--global | --local] [--profile SOURCE]
                 [--harness claude-code|cursor|qwen]
...
The `qwen` hook adapter always exits `0`: it signals a deny only through its
decision JSON, because Qwen treats a non-zero exit code (`2`) as a block and fails
open otherwise — so an internal read/parse error can never become a deny.
...
just test-qwen       # drives `qwen --yolo`; writes/reads ~/.qwen/settings.json
```
...
`PATH`. The `qwen` check additionally proves the deny holds even under `--yolo` —
the hook fires in every approval mode, so it is the sole gate.
