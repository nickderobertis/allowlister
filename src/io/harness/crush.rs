//! Crush `PreToolUse` hook adapter.
//!
//! Reads the hook JSON from stdin, evaluates the `bash` command, and — only when
//! the verdict is **deny** — writes Crush's native decision on stdout
//! (`{"decision":"deny","reason":"…"}`). Three protocol facts shape the rest:
//!
//! 1. **The hook runs before Crush's permission check, and `crush run`
//!    (headless) auto-approves the whole session.** So in an unattended run the
//!    hook is the *only* thing that can stop a command: a `deny` here is
//!    authoritative even though every command is otherwise auto-approved.
//! 2. **Only `deny` blocks.** Crush maps any decision other than `allow`/`deny`
//!    to "no opinion", and an explicit `allow` would *pre-approve* the command
//!    and skip its prompt. An allowlister gate only ever wants to block, so an
//!    allow or defer verdict emits *nothing*: empty stdout is a true
//!    fall-through to Crush's normal flow, and a `deny` is the only thing we
//!    assert.
//! 3. **Exit code is always `0`.** Crush blocks on exit `2` (its block code) and
//!    fails *open* on every other non-zero exit, so a deny must travel as JSON,
//!    not as an exit code. Always exiting `0` means our own read/parse failure
//!    is a no-op that lets the call through — an internal error can neither
//!    become a block nor be mistaken for one.
//!
//! The command arrives under `tool_input.command`; Crush names its shell tool
//! `bash` (lowercase). Any other tool emits nothing.
...
/// The canonical tool name Crush uses for shell commands (lowercase). Any other
/// tool is not one we gate, so it emits nothing.
...
/// so our own failures cannot become a Crush block (Crush fails open on any exit
/// other than its block code). Separated from [`run`] so the protocol can be
...
pub fn evaluate<R: Read, W: Write, E: Write>(mut stdin: R, mut stdout: W, mut stderr: E) -> i32 {
    ...
    if let Err(err) = stdin.read_to_string(&mut buffer) {
        // Never block on our own failure: empty stdout + exit 0 lets Crush run
        // its normal (auto-approving, in headless) flow — fail open.
    ...
    if input.tool_name != SHELL_TOOL {
        // Not a shell command — let Crush's normal flow handle it (emit nothing).
        return 0;
    }
    ...
    // fall-through to Crush's own flow — because an explicit `allow` would
    // pre-approve and skip its prompt, which an allowlister gate must not do.
...
/// The directory used for project-config discovery. Crush sends the session `cwd`;
...
/// Extract the shell command from `tool_input`. Crush sends it as a JSON object
...
/// Write Crush's native flat `deny` decision. The reason is carried so the model
/// sees why the command was blocked.
fn write_deny<W: Write>(stdout: &mut W, reason: &str) {
    let output = HookOutput {
        decision: "deny",
        reason: reason.to_string(),
    };
    // This small fixed shape cannot fail to serialize; if the write fails Crush
...
mod tests {
    ...
    fn invalid_json_defers_via_exit_zero_and_empty_stdout() {
        // The fail-open path: a parse error must NOT exit 2 (which Crush would
    ...
    fn non_bash_tool_emits_nothing() {
        let (code, stdout) = run_payload(r#"{"tool_name":"view","tool_input":{"command":"x"}}"#);
        assert_eq!(code, 0);
    ...
    fn unknown_command_defers_with_empty_stdout() {
        ...
        assert_eq!(code, 0);
        // Crush falls through on empty stdout, so a defer emits nothing.
    ...
    fn allowed_command_emits_nothing() {
        ...
        // pre-approve and skip Crush's prompt, which a gate must not do.
        let dir = TempDir::new().unwrap();
        fs::create_dir(dir.path().join(".git")).unwrap();
        fs::write(
            dir.path().join(".allowlister.json"),
