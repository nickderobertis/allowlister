...
pub fn run() -> Result<i32> {
    ...
    } else if request.command.contains("plugin-project") {
        // Echo the raw `cwd` alongside the resolved `project` identity so an e2e
        // test can confirm the request carries the git-repository tag (remote /
        // repo root / cwd fallback), not just the literal directory. Deny so the
        // echoed reason surfaces regardless of the base verdict.
        (
            "deny",
            format!(
...
fn run_tool(request: &Request) -> Result<i32> {
    ...
    } else if saw("tool-project") {
        // The tool-subject counterpart of the `plugin-project` shell marker: echo
        // the raw `cwd` and the resolved `project` identity so an e2e test can
        // confirm the tool request carries the repository tag too.
        (
            "deny",
            format!(
                "project echo: cwd={} project={}",
