...
use crate::io::hooks::{self, HookOutcome};
...
/// The OpenCode plugin shim `init --harness opencode --no-hooks` prints for
/// manual wiring. OpenCode blocks a tool call only from an in-process plugin, so
/// this is a full file to drop in: it spawns `allowlister hook opencode` before
/// every tool call, pipes the tool name and args as JSON, and throws to block
/// when allowlister denies — failing open if the binary cannot run. Auto-loaded
/// from `.opencode/plugin/` (project) or `~/.config/opencode/plugin/` (global).
const OPENCODE_PLUGIN_SNIPPET: &str = r#"// allowlister OpenCode plugin — generated by `allowlister init --harness opencode`.
//
...
// other tool (read/write/edit/webfetch and MCP `server_tool`) to the tool-rule
// engine. Edit your rules in your allowlister config (.allowlister.jsonc), not here;
// re-running init regenerates this file. Auto-loaded from .opencode/plugin/ (project)
// or ~/.config/opencode/plugin/ (global) with no opencode.json entry needed.
...
/// Register the chosen harness's hook and report what changed. The install
/// itself is harness-agnostic (one [`crate::io::hooks`] call); only the
/// user-facing wording differs per harness.
fn register_hook_for<W: Write>(
    ...
    let outcome = hooks::install_hook(harness, global, cwd, env)?;
    report_hook(out, harness, &outcome);
    Ok(())
}
...
/// Report what registering the chosen harness's hook changed. The phrasing — the
/// event it registered on, any post-install note, and (Claude only) the allow-list
/// warning — is per harness; the create/update/no-op shape is shared.
fn report_hook<W: Write>(out: &mut W, harness: Harness, outcome: &HookOutcome) {
    let command = format!("allowlister hook {}", hooks::harness_id(harness));
    let _ = writeln!(out);
    if outcome.was_noop() {
        // OpenCode writes a plugin file rather than registering a hook entry, so
        // its no-op reads "Plugin already up to date" instead of "registered".
        let (noun, state) = match harness {
            Harness::OpenCode => ("Plugin", "up to date"),
            _ => ("Hook", "registered"),
        };
        let _ = writeln!(
            out,
            "{noun} already {state} in {} (nothing to change).",
        ...
        let _ = writeln!(
            ...
            "{verb} {}: {}",
            outcome.path.display(),
            hook_action(harness, &command)
        );
        if outcome.denies_added > 0 {
            let _ = writeln!(
                out,
                "  Added {} nuclear-pattern deny rule(s) as a backstop.",
                outcome.denies_added
            );
        }
        if let Some(note) = hook_note(harness) {
            let _ = writeln!(out, "{note}");
        }
    }
    // Only Claude Code has a permissions allow list a careless edit could use to
    // short-circuit the hook, so the warning is Claude-only — and printed even on
    // a no-op, since the risk is in the file regardless of what this run changed.
...
/// The clause describing what was registered, naming the harness's gated event
/// and the command. OpenCode writes a plugin, so its phrasing differs.
fn hook_action(harness: Harness, command: &str) -> String {
    match harness {
        Harness::ClaudeCode => format!("registered '{command}' as the Bash PreToolUse hook."),
        Harness::Cursor => {
            format!("registered '{command}' as the beforeShellExecution hook.")
        }
        Harness::Codex => format!("registered '{command}' as the Bash PreToolUse hook."),
        Harness::Crush => format!("registered '{command}' as the bash PreToolUse hook."),
        Harness::Qwen => {
            format!("registered '{command}' as the run_shell_command PreToolUse hook.")
        }
        Harness::Goose => format!("registered '{command}' as the shell PreToolUse hook."),
        Harness::OpenCode => {
            format!("wrote the OpenCode plugin that gates bash via '{command}'.")
        }
        Harness::Copilot => format!("registered '{command}' as the preToolUse hook."),
    }
}

/// A harness-specific reminder printed after a fresh install, where one helps:
/// Codex trusts new hooks on startup, and Goose/OpenCode load on their next start.
fn hook_note(harness: Harness) -> Option<&'static str> {
    match harness {
        Harness::Codex => Some(
            "  Codex reviews new hooks on startup — approve it when prompted to activate the gate.",
        ),
        Harness::Goose => {
            Some("  Goose discovers the plugin on its next start — no enable flag or trust step.")
        }
        Harness::OpenCode => {
            Some("  OpenCode loads it on its next start — no opencode.json entry needed.")
        }
        _ => None,
    }
}
...
fn write_opencode_hook_setup<W: Write>(out: &mut W) -> io::Result<()> {
    ...
    writeln!(out)?;
    writeln!(out, "{OPENCODE_PLUGIN_SNIPPET}")
}
